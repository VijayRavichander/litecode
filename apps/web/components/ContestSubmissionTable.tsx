...
export const ContestSubmissionTable = ({ userId, contestProblemID }: SubmisionProps) => {
  ...
  useEffect(() => {
    const getSumbissions = async (userId, contestProblemID, limit, offset) => {
      const res = await axios.get(`/api/v1/contest/submissions`, {
        params: {
          userId,
          contestProblemID,
          limit,
    ...
    getSumbissions(userId, contestProblemID, limit, offset);
  }, [offset]);
        ...
        <TableBody>
          {submissions &&
            submissions.map((submission: Submissions) => (
                <SubmisionRow key={submission.id} submission={submission.submission}/>
            ))}
        </TableBody>
      </Table>
